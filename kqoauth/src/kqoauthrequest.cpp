...
QString KQOAuthRequestPrivate::oauthSignature()  {
    ...
    if(oauthSignatureMethod == "PLAINTEXT")
        return QString( QUrl::toPercentEncoding(oauthConsumerSecretKey + "&" + oauthTokenSecret) );
    else
...
void KQOAuthRequest::initRequest(KQOAuthRequest::RequestType type, const QUrl &requestEndpoint) {
    ...
    this->setSignatureMethod(KQOAuthRequest::PLAINTEXT);
